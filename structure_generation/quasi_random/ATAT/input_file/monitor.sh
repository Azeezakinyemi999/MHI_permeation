...
# --- ATAT binary path ---
ATAT_BIN="/Users/akinyemi.az/Desktop/HE_papers/yoshi/Analytical_model/MHI_permeation/atat-sqs-gui/atat/build/bin"
export PATH="${ATAT_BIN}:${PATH}"
# Auto-generated script with embedded file creation
# Generated configuration: mp-23_Ni.cif, 5×5×5, 500 atoms
...
0 1 0
0 0 1
0.000000 0.000000 0.000000 Al=0.020000,B=0.020000,C=0.020000,Cr=0.084000,Fe=0.036000,Mo=0.100000,Ni=0.720000
0.000000 0.500000 0.500000 Al=0.020000,B=0.020000,C=0.020000,Cr=0.084000,Fe=0.036000,Mo=0.100000,Ni=0.720000
0.500000 0.000000 0.500000 Al=0.020000,B=0.020000,C=0.020000,Cr=0.084000,Fe=0.036000,Mo=0.100000,Ni=0.720000
0.500000 0.500000 0.000000 Al=0.020000,B=0.020000,C=0.020000,Cr=0.084000,Fe=0.036000,Mo=0.100000,Ni=0.720000
EOF
...
start_parallel_monitoring_process() {
   ...
   echo "Monitor started for 3 parallel runs. Waiting for 5 seconds to allow mcsqs to initialize..."
   sleep 5
   ...
   echo "----------------------------------------"
   echo "Monitoring 3 parallel MCSQS runs every minute"
   echo "Log files: mcsqs1.log, mcsqs2.log, ..., mcsqs3.log"
   echo "----------------------------------------"
...
check_prerequisites() {
   ...
   echo "Command: corrdump -l=rndstr.in -ro -noe -nop -clus -2=2.0"
   corrdump -l=rndstr.in -ro -noe -nop -clus -2=2.0
   if [ $? -ne 0 ]; then
...
cleanup() {
   ...
   if [ -f "$PROGRESS_FILE" ]; then
       ...
       best_objective=$(echo "$last_line" | cut -d',' -f$((3 + 3 * 3)))
       best_run=$(echo "$last_line" | cut -d',' -f$((4 + 3 * 3)) | sed 's/Run//')
   fi
...
echo "================================================"
echo "Configuration:"
echo "  - Structure: mp-23_Ni.cif"
echo "  - Supercell: 5×5×5 (500 atoms)"
echo "  - Parallel runs: 3"
...
if [ -f "$PROGRESS_FILE" ]; then
   ...
   echo "  - Best overall objective:  $(tail -1 "$PROGRESS_FILE" | cut -d',' -f$((3 + 3 * 3)))"
fi
